...
def crawler_daemon(g): 
	while True: 
		...
		for aPeer in g.peers: 
			try: 		
				crawledPeers = OcsadURLRetriever.retrieveURL(f"http://[{aPeer}]:55227/listpeers", 320000, 120, True, 1024, None, 300).split("\n")
				newnewpeers = []
				...
				print(f"crawler: {aPeer} didn't reply upon network crawl"+str(e))
				...
				if len(flacsize)>0: 
					...
					for i in catalog: 
						if p!='' and i!='' and i!=p:
							...
							if self.dling and not os.path.exists(os.path.join(finaldir, os.basename(i))):
								try: 
									temp = OcsadURLRetriever.retrieveURL("http://["+p+"]:55228/flac?"+urllib.parse.quote(i, safe=''),4000000000 ,800, False)
...
class OcsadURLRetriever:
    @staticmethod
    def retrieveURL(url, max_length=32000, reqtimeout=800, decode=True, iteration=1024, text_setter=None, total_timeout=120):
        # Conteneurs pour passer les données entre le thread et le programme principal
        result_holder = {"data": None, "exception": None}
        request_ref = {"resp": None} # Pour pouvoir couper la connexion depuis l'extérieur

        def _worker():
            try:
                # Démarrage de la requête
                r = requests.get(url, timeout=reqtimeout, stream=True)
                request_ref["resp"] = r # On garde la ref pour le close()
                ...
                # Succès : on stocke le résultat
                if decode:
                    result_holder["data"] = char_array.decode("utf-8")
                else:
                    result_holder["data"] = char_array
                    
            except Exception as e:
                # Echec : on stocke l'exception pour la relever dans le thread principal
                result_holder["exception"] = e

        # 1. On lance le worker dans un thread classique
        t = threading.Thread(target=_worker)
        # daemon=True assure que le thread sera tué si le programme principal quitte brutalement
        t.daemon = True 
        t.start()

        # 2. On attend la fin du thread avec le timeout global
        t.join(timeout=total_timeout+reqtimeout)

        # 3. Vérification : Le thread est-il toujours vivant après le timeout ?
        if t.is_alive():
            # C'est le TIMEOUT !
            # On coupe brutalement la connexion pour débloquer le worker
            if request_ref["resp"]:
                try:
                    request_ref["resp"].close()
                except:
                    pass
            raise ValueError("Total timeout exceeded")

        # 4. Si le thread a fini, on regarde s'il a levé une erreur
        if result_holder["exception"]:
            # On relève l'erreur qui s'est produite dans le thread (ex: 404, DNS error, ou max_length)
            error = result_holder["exception"]
            # Si c'est déjà un ValueError (notre max_length), on le laisse tel quel
            if isinstance(error, ValueError):
                raise error
            # Sinon on peut masquer l'erreur ou la laisser remonter
            # Ici je garde votre logique originale :
            if isinstance(error, requests.exceptions.Timeout):
                 raise ValueError("Invalid Ocsad URL")
            # Pour tout autre crash inattendu
            raise error
		...
		try:
			...
			if path=="/podcast":
				...
				<channel>
					...
					<link>"""+link+"""</link>
				"""
				if not g.podcaster.donation_url is None and not g.podcaster.donation_name is None:
					pod = pod + """<podcast:funding url=\""""+html.escape(g.podcaster.donation_url)+"""\"><![CDATA["""+g.podcaster.donation_name+"""]]≥</podcast:funding>
					"""
				...
				if query!='':
					...
					for mp3 in files: 
						if mp3.name.endswith(".mp3"):
							...
							track = None
							year = None
							while title is None or album is None or artist is None or file is None or duration is None or year is None or genre is None or track is None or comment is None:
								...
								if os.path.exists(os.path.join(datadir, mp3.name+".duration.txt")):
									...
									with open(os.path.join(unshareddatadir,mp3.name+'.duration.txt'), 'r') as myfile:
										duration=myfile.read()
								if os.path.exists(os.path.join(datadir, mp3.name+".track.txt")):
									with open(os.path.join(datadir,mp3.name+'.track.txt'), 'r') as myfile:
										track=myfile.read()
								elif os.path.exists(os.path.join(unshareddatadir, mp3.name+".track.txt")):
									with open(os.path.join(unshareddatadir,mp3.name+'.track.txt'), 'r') as myfile:
										track=myfile.read()
								if os.path.exists(os.path.join(datadir, mp3.name+".year.txt")):
									with open(os.path.join(datadir,mp3.name+'.year.txt'), 'r') as myfile:
										year=myfile.read()
								elif os.path.exists(os.path.join(unshareddatadir, mp3.name+".year.txt")):
									with open(os.path.join(unshareddatadir,mp3.name+'.year.txt'), 'r') as myfile:
										year=myfile.read()
								if os.path.exists(os.path.join(datadir, mp3.name+".genre.txt")):
									with open(os.path.join(datadir,mp3.name+'.genre.txt'), 'r') as myfile:
										genre=myfile.read()
								elif os.path.exists(os.path.join(unshareddatadir, mp3.name+".genre.txt")):
									with open(os.path.join(unshareddatadir,mp3.name+'.genre.txt'), 'r') as myfile:
										genre=myfile.read()
								if os.path.exists(os.path.join(datadir, mp3.name+".comment.txt")):
									with open(os.path.join(datadir,mp3.name+'.comment.txt'), 'r') as myfile:
										comment=myfile.read()
								elif os.path.exists(os.path.join(unshareddatadir, mp3.name+".comment.txt")):
									with open(os.path.join(unshareddatadir,mp3.name+'.comment.txt'), 'r') as myfile:
										comment=myfile.read()								
								file = mp3.name
						
								#tag scanner hasn't still processed this file, sleeping
								if title is None or album is None or artist is None or file is None or duration is None or year is None or genre is None or track is None or comment is None:
									sleep(5)
			...
			if path=='/ts':
				chemins = [os.path.join(os.path.join(basedir, "MetadataShares"), f) for f in os.listdir(os.path.join(basedir, "MetadataShares"))]
				fichiers = [f for f in chemins if os.path.isfile(f)]

				if fichiers:
					# Initialisation des valeurs pour le calcul du checksum
					hash_sha256 = hashlib.sha256()

					for f in fichiers:
						# Récupère la date de modification
						mtime = os.path.getmtime(f)
						# Met à jour le checksum avec la date de modification convertie en bytes
						hash_sha256.update(str(mtime).encode('utf-8'))

					# Calcul du digest final
					checksum = hash_sha256.hexdigest()
					self.wfile.write(checksum.encode('utf-8'))
